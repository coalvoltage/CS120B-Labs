...
unsigned char initString[] = "Congratulations!";
unsigned char outputKeypad = 'C';
unsigned char cursorIndex = 1;
...
enum LEDStates {LEDStart, LEDInit, LEDOutput};
unsigned char TickOutputLED(unsigned char state) {
	...
	switch (state){
		case LEDInit:
		LCD_DisplayString(1,initString);
		break;
		case LEDOutput:
